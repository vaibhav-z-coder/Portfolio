# 🎨 Vaibhav Singh — Portfolio

🔗 **Live Website:** https://portfolio-nu-ten-73.vercel.app

This is my personal portfolio showcasing my UI/UX design skills and frontend development work. It highlights my major projects, technical skills, and experience building responsive landing pages and clone sites.

---

## 👨‍💻 About Me

Hi, I’m **Vaibhav Singh** —a Web Designer with a passion for creating clean, responsive, and user-friendly websites.

**Role:** UI/UX Designer  
**Focus:** Frontend design, rapid prototyping, responsive layouts  
**Currently learning:**web development fundamentals

---

## 💼 Major Projects

Here are a few of my notable works:

- **Enjoy Algorithms** — Interactive algorithm learning landing page  
- **Namecheap Clone** — Pixel-perfect replica of the Namecheap homepage  
- **GoDaddy Clone** — Responsive clone of GoDaddy’s landing page

Each project demonstrates modern CSS grid and responsive design techniques.

---

## 🛠️ Technical Skills

- **Frontend:** HTML5, CSS3, Responsive Web Design  
- **Tools:** Git, GitHub 

---

## 📂 Project Highlights

Some additional sites I’ve built and practiced with:

- Netflix landing page clone  
- YouTube homepage clone

These helped improve my layout structuring and design responsiveness.

---

## 📫 Contact Me

I’m always open to new opportunities and collaborations!

- **LinkedIn:** https://linkedin.com/in/vaibhav-singh  
- **GitHub:** https://github.com/your-github  
- **Email:** vaibhav6240018@gmail.com

---

## ⚙️ Deployment

This portfolio is deployed using **Vercel** for fast, hosting.

---

## 📄 License

This project is open source — feel free to explore, remix, or use it as inspiration for your own portfolio 🚀.
